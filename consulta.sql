...
SELECT
    ...
    CASE
        WHEN TRUNC(SYSDATE - MAX(demi_not)) >= 0 AND TRUNC(SYSDATE - MAX(demi_not)) <= 30 THEN 'Muito Recente'
        WHEN TRUNC(SYSDATE - MAX(demi_not)) >= 31 AND TRUNC(SYSDATE - MAX(demi_not)) <= 60 THEN 'Recente'
        WHEN TRUNC(SYSDATE - MAX(demi_not)) >= 61 AND TRUNC(SYSDATE - MAX(demi_not)) <= 90 THEN 'Média Recência'
        WHEN TRUNC(SYSDATE - MAX(demi_not)) >= 91 AND TRUNC(SYSDATE - MAX(demi_not)) <= 120 THEN 'Antigo'
        WHEN TRUNC(SYSDATE - MAX(demi_not)) >= 120 THEN 'Muito Antigo'
